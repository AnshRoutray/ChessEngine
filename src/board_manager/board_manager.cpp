...
void init_zobrist_hashes() {
  ...
  for (int color = 0; color < 2; color++) {
    for (int piece = PAWN_PIECE; piece < 7; piece++) {
      for (int sq = 0; sq < 64; sq++) {
        ZOBRIST_VALUES[color][piece][sq] = rng();
      }
    }
  }
  ZOBRIST_TURN = rng();
  for (int color = 0; color < 2; color++) {
    for (int state = 0; state < 4; state++) {
      ZOBRIST_CASTLE[color][state] = rng();
    }
  }
...
Board::Board(uint64_t pawns, uint64_t knights, uint64_t bishops, uint64_t rooks,
             ...
             uint64_t enemy_knights, uint64_t enemy_bishops,
             uint64_t enemy_rooks, uint64_t enemy_queen, uint64_t enemy_king,
             uint8_t castle_state[2], uint8_t turn, Move previous_move)
    : pawns(pawns), knights(knights), bishops(bishops), rooks(rooks),
      queen(queen), king(king), enemy_pawns(enemy_pawns),
...
void Board::init_piece_locations_and_hash() {
  ...
  zobrist_hash ^= ZOBRIST_CASTLE[0][castle_state[0]];
  zobrist_hash ^= ZOBRIST_CASTLE[1][castle_state[1]];
  if (turn == BLACK_TURN) {
...
UndoInfo Board::playMove(Move move) {
  ...
  // If the previous move was a pawn double push, the EP-target hash it set
  // expires now. Clear it before applying this move.
  {
    int prev_to = GET_TO_SQUARE(previous_move);
    int prev_from = GET_FROM_SQUARE(previous_move);
    if ((enemy_pawns & (1ULL << prev_to)) && abs(prev_to - prev_from) == 16) {
  ...
  *piece &= ~(1ULL << from_square);
  ...
  // Zobrist hash
  zobrist_hash ^= ZOBRIST_VALUES[turn][piece_type][from_square];
  zobrist_hash ^= ZOBRIST_VALUES[turn][piece_type][to_square];
  ...
  if (castle_state[1] != info.previous_castle_state[1]) {
    zobrist_hash ^= ZOBRIST_CASTLE[1][info.previous_castle_state[1]];
    zobrist_hash ^= ZOBRIST_CASTLE[1][castle_state[1]];
  }
  // EP target if this move was a pawn double push
  if (piece_type == PAWN_PIECE && abs(to_square - from_square) == 16) {
    zobrist_hash ^= ZOBRIST_EP_FILE[to_square % 8];
  }
  // Side to move flips
  zobrist_hash ^= ZOBRIST_TURN;
  previous_move = move;
...
uint16_t Board::generateCaptureMoves(Move moveList[MAX_MOVES]) {
  uint16_t moveIndex = 0;

  // Pawn captures (diagonal attacks only)
  uint64_t diagonal_pawn_attacks_left = 0;
  uint64_t diagonal_pawn_attacks_right = 0;
  if (turn) {
    diagonal_pawn_attacks_left = ((pawns & ~COLUMN[7]) << 9) & enemy_pieces;
  ...
  while (temp_knights) {
    index = __builtin_ctzll(temp_knights);
    uint64_t knight_attacks = KNIGHT_ATTACKS[index] & enemy_pieces;
    while (knight_attacks) {
      uint8_t attack_index = __builtin_ctzll(knight_attacks);
      moveList[moveIndex++] = ENCODE_MOVE(attack_index, index, 0, 0);
      knight_attacks &= knight_attacks - 1;
    }
    temp_knights &= temp_knights - 1;
  ...
  // Diagonal captures (bishops and queens)
  uint64_t diagonal_pieces = queen | bishops;
  while (diagonal_pieces) {
    index = __builtin_ctzll(diagonal_pieces);
    uint16_t key = _pext_u64(friendly_pieces | enemy_pieces, DIAGONALS[index]);
    uint64_t attack_bitboard = DIAGONAL_ATTACKS[index][key] & enemy_pieces;
    while (attack_bitboard) {
      uint8_t temp_index = __builtin_ctzll(attack_bitboard);
      moveList[moveIndex++] = ENCODE_MOVE(temp_index, index, 0, 0);
      attack_bitboard &= attack_bitboard - 1;
  ...
  // Straight captures (rooks and queens)
  uint64_t straight_pieces = queen | rooks;
  while (straight_pieces) {
    index = __builtin_ctzll(straight_pieces);
    uint16_t key = _pext_u64(friendly_pieces | enemy_pieces,
  ...
  // King captures only
  uint8_t king_square = __builtin_ctzll(king);
  uint64_t king_attacks = KING_ATTACKS[king_square] & enemy_pieces;
  while (king_attacks) {
    index = __builtin_ctzll(king_attacks);
    moveList[moveIndex++] = ENCODE_MOVE(index, king_square, 0, 0);
    king_attacks &= king_attacks - 1;
...
bool Board::operator==(Board other_board) {
  ...
  bool pieces_equal = pawns_equal && enemy_pawns_equal && knights_equal &&
                      enemy_knights_equal && bishops_equal &&
                      enemy_bishops_equal && rooks_equal && enemy_rooks_equal &&
                      queens_equal && enemy_queens_equal && king_equal &&
                      enemy_king_equal;
  ...
  bool castling_states_equal =
      (castle_state[0] == other_board.castle_state[0]) &&
