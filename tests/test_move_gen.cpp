...
bool testPosition(const char *description, Board &board, int expected) {
  ...
  if (count != expected) {
    ...
    std::cerr << "  Expected " << expected << " moves, got " << count
              << std::endl;
    return false;
  }

  std::cout << "PASSED: " << description << " (" << count << " moves)"
            << std::endl;
  return true;
}
...
int main() {
  ...
  // White pawn on g7, can promote to 4 pieces = 4 moves
  {
    ...
    uint8_t castle_state[2] = {CASTLE_NO_SHORT_NO_LONG,
                               CASTLE_NO_SHORT_NO_LONG};
    uint8_t turn = 1; // white to move
    Move previous_move = Move();

    Board board(pawns, knights, bishops, rooks, queen, king, enemy_pawns,
                enemy_knights, enemy_bishops, enemy_rooks, enemy_queen,
                enemy_king, castle_state, turn, previous_move);

    allPassed &= testPosition("Pawn promotion (straight)", board,
                              9); // 4 promotions + king moves
  }
  ...
  // White pawn on g7, black pieces on f8 and h8
  {
    ...
    allPassed &= testPosition("Pawn capture-promotions", board,
                              13); // 4+4+4 promotions + 1 king
  }
  ...
  // White pawn on e5, black pawn just moved d7-d5
  {
    ...
    allPassed &= testPosition("En passant available", board,
                              3); // e5-e6, e5xd6 (en passant), king move
  }
  ...
  // Test 4: Knight in corner (only 2 moves)
  {
    ...
    allPassed &= testPosition("Knight in corner", board,
                              10); // 2 knight moves + 8 king moves
  }
  ...
  // Test 5: Knight in center (maximum mobility - 8 moves)
  {
    ...
    allPassed &= testPosition("Knight in center", board,
                              16); // 8 knight moves + 8 king moves
  }
  ...
  // Test 6: Castling kingside available
  {
    ...
    allPassed &= testPosition("Kingside castling available", board,
                              20); // lots of rook+king moves + O-O
  }
  ...
  // Test 7: Castling queenside available
  {
    ...
    allPassed &= testPosition("Queenside castling available", board,
                              22); // rook+king moves + O-O-O
  }
  ...
  // Test 8: Bishop on open diagonal
  {
    ...
    allPassed &= testPosition("Bishop on open diagonal", board,
                              21); // 13 bishop moves + 8 king moves
  }
  ...
  // Test 9: Rook on open file
  {
    ...
    allPassed &= testPosition("Rook on open board", board,
                              22); // 14 rook moves + 8 king moves
  }
  ...
  // Test 10: Queen in center (maximum mobility)
  {
    ...
    allPassed &= testPosition("Queen in center", board,
                              35); // 27 queen moves + 8 king moves
  }
